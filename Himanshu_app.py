...
def send_message(contact, msg, via="sms"):
    try:
        if via == "whatsapp":
            from_num = "whatsapp:" + TWILIO_NUMBER
            to_num = "whatsapp:" + contact
        else:
...
def add_user(code, name, father, address, contact, admission_date, monthly_fee):
    ...
    conn.commit()
    conn.close()
    send_message(contact, f"Welcome {name}! Your Library Code is {code}", "whatsapp")
...
if choice=="Admin - Add User":
    ...
    if st.button("Add User"):
        add_user(code,name,father,address,contact,str(admission_date),fee)
        ...
        if not dues.empty:
            ...
            if st.button("📩 Send Reminders"):
                for _,row in dues.iterrows():
                    msg=f"Dear {row['name']}, your library due is ₹{row['Due']}."
                    send_message(row["contact"],msg,"sms")
                    send_message(row["contact"],msg,"whatsapp")
